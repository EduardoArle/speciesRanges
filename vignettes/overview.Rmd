...
3 - Updata the range map based on point records representing occurrence records - presences and/or absences, and specialist range maps when avaiable.

```{r, warning = FALSE}
new_range <- updateRange(occ, range)
```

Running time: ≈ 2s.
Output: a shapefile representing an updated range map based on the convex hull using the occurrence points. The map will be pixelated according to the size of the grain cell.
